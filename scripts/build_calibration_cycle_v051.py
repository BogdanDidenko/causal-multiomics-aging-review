...
SOURCE = BENCHMARKS / "title_abstract_regression_v0.41.0_remaining_66.csv"
ACCESSED = [
...
HOLDOUT = BENCHMARKS / "title_abstract_stability_holdout_v6_v0.51.0_25.csv"
REMAINDER = BENCHMARKS / "title_abstract_regression_v0.51.0_remaining_41.csv"
MANIFEST = BENCHMARKS / "calibration_cycle_v0.51.0_manifest.json"
SEED = "causal-multiomics-aging|sealed-holdout-v6|v0.51.0"
...
def main() -> None:
    ...
    if len(remainder_rows) != 41:
        raise ValueError(
            f"Expected 41 untouched remainder records, found {len(remainder_rows)}"
        )
    if holdout_ids & accessed_ids:
        raise ValueError("New holdout overlaps an accessed set")
    if holdout_ids & ids(remainder_rows):
    ...
    print(
        "cycle_v0.51.0 "
        f"source={len(source_rows)} sealed={len(holdout_rows)} "
        f"remainder={len(remainder_rows)}"
    )
