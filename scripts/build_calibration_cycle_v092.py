...
SOURCE = BENCHMARKS / "title_abstract_regression_v0.51.0_remaining_41.csv"
ACCESSED = [
    ...
    BENCHMARKS / "title_abstract_stability_holdout_v6_v0.51.0_25.csv",
]
DEVELOPMENT = BENCHMARKS / "title_abstract_calibration_v0.92.0_16.csv"
HOLDOUT = BENCHMARKS / "title_abstract_stability_holdout_v7_v0.92.0_25.csv"
MANIFEST = BENCHMARKS / "calibration_cycle_v0.92.0_manifest.json"
SEED = "causal-multiomics-aging|sealed-holdout-v7|v0.92.0"
...
def main() -> None:
    ...
    accessed_metadata: list[dict[str, object]] = []
    accessed_ids: set[str] = set()
    for path in ACCESSED:
        _, rows = read_rows(path)
        if source_ids & ids(rows):
            raise ValueError(f"Untouched source overlaps accessed set: {path}")
        accessed_ids.update(ids(rows))
        accessed_metadata.append(
            {
    ...
    development_rows = sorted(
        [row for row in source_rows if row["canonical_id"] not in holdout_ids],
        key=lambda row: row["canonical_id"],
    )
    if len(development_rows) != 16:
        raise ValueError(
            f"Expected 16 development records, found {len(development_rows)}"
        )
    if holdout_ids & accessed_ids:
        raise ValueError("New holdout overlaps an accessed set")
    if holdout_ids & ids(development_rows):
        raise ValueError("New holdout overlaps the development set")

    write_rows(DEVELOPMENT, fieldnames, development_rows)
    write_rows(HOLDOUT, fieldnames, holdout_rows)
    manifest = {
        "cycle_version": "0.92.0",
        ...
        "excluded_accessed_sets": accessed_metadata,
        "development_set": {
    ...
    print(
        "cycle_v0.92.0 "
        f"source={len(source_rows)} development={len(development_rows)} "
        f"sealed={len(holdout_rows)}"
    )
