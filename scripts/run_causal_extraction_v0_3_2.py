#!/usr/bin/env python3
"""Run the frozen v0.3.2 role-contract causal extraction ablation."""
...
DEFAULT_SUITE = REPO / "protocol/causal_extraction/v0.3.2"
DEFAULT_OUTPUT = (
    REPO / "data/causal_extraction/v0.3.2_development/terra_5repeat"
)
...
class V032Runner:
    def __init__(
        ...
        *,
        ...
        eligible = read_json(
            self.suite / str(self.runtime["eligible_candidate_set"])
        )
        self.eligible_ids = {
        ...
        configured_roles = {
            role["role_id"]: role for role in self.runtime["roles"]
        }
        selected_role_ids = roles or list(configured_roles)
        unknown_roles = set(selected_role_ids) - set(configured_roles)
        if unknown_roles:
            raise ValueError(f"Unknown roles: {sorted(unknown_roles)}")
        self.roles = [configured_roles[role_id] for role_id in selected_role_ids]
        self.prompt_template = (self.suite / self.runtime["prompt"]).read_text(
            encoding="utf-8"
        )
    ...
    def verify_freeze(self) -> None:
        ...
        if self.freeze["artifact_manifest_sha256"] != sha256_file(manifest_path):
            raise ValueError("v0.3.2 artifact manifest changed after freeze")
        if self.freeze["status"] != "frozen_before_first_v0_3_2_terra_output":
            raise ValueError("v0.3.2 suite is not frozen for model execution")
        for group in ("artifacts", "source_data", "parent_outputs"):
    ...
    def preflight(self) -> None:
        ...
        if not self.resume and git_dirty():
            raise ValueError("Git worktree must be clean before a new v0.3.2 run")
    ...
    def build_calls(self) -> list[CallSpec]:
        ...
        for sampled in self.sample["reports"]:
            ...
            if packet_atom_ids(packet) != expected_atom_ids:
                raise ValueError(f"Compact packet mismatch: {document_id}")
            all_candidates = self.scaffold_reports[document_id]
            for role in self.roles:
                ...
                candidate_report = (
                    ...
                    if role["candidate_scope"] == "all_fixed_candidates"
                        ...
                        all_candidates, self.eligible_ids[document_id]
                    )
                )
                errors = validate_candidate_scaffold(candidate_report, atom_index)
                if errors:
                    raise ValueError(
                        f"Invalid {role_id} scaffold for {document_id}: {errors}"
                    )
                rendered_scaffold = render_candidate_scaffold(candidate_report)
                contract = (self.suite / role["contract"]).read_text(encoding="utf-8")
                ...
                if token_count(prompt) > int(
                    self.runtime["max_rendered_prompt_tokens"]
                ):
                    raise ValueError(
                        f"Rendered prompt exceeds token limit: {role_id}/{document_id}"
                    )
                source_schema = read_json(self.suite / role["schema"])
                input_dir = self.output / "inputs" / role_id / document_id
                ...
                write_json(
                    input_dir / "input_manifest.json",
                    {
                        ...
                        "candidate_scaffold_sha256": sha256_text(
                            rendered_scaffold
                        ),
    ...
    def run_call(self, spec: CallSpec) -> dict[str, Any]:
        ...
        write_json(
            spec.call_dir / "request.json",
            {
                ...
                "schema_sha256": sha256_text(canonical_json(spec.source_schema)),
        ...
        for attempt in range(1, maximum_attempts + 1):
            ...
            try:
                ...
                parsed, raw = self.provider.complete_json(
                    spec.prompt,
                    spec.runtime_schema,
                    schema_name=f"causal_{spec.role_id}_v0_3_2",
                )
                schema_errors = validate_schema(parsed, spec.source_schema)
                semantic_errors = (
                    validate_role_response(
                        parsed,
