...
from causal_multiomics_aging_review.config import DEFAULT_SUITE_CONFIG, load_stage_config
...
def _validate_run_manifest(
    ...
    expected_suite_hash = sha256_file(suite_config_path.resolve())
    if manifest.get("suite_config_sha256") != expected_suite_hash:
        raise SystemExit(
            f"Run manifest {manifest_path} used suite hash "
            f"{manifest.get('suite_config_sha256')!r}, "
            f"expected {expected_suite_hash!r}"
        )
