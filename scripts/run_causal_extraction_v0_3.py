#!/usr/bin/env python3
"""Run the frozen v0.3 report-level causal-analysis inventory experiment."""
...
DEFAULT_SUITE = REPO / "protocol/causal_extraction/v0.3.0"
DEFAULT_OUTPUT = REPO / "data/causal_extraction/v0.3.0_development/terra_5repeat"
...
class V03Runner:
    def __init__(
        ...
        *,
        ...
        self.source_schema = read_json(
            self.suite / str(self.runtime["schema"])
        )
    ...
    def verify_freeze(self) -> None:
        ...
        if self.freeze["artifact_manifest_sha256"] != sha256_file(manifest_path):
            raise ValueError("v0.3 artifact manifest changed after freeze")
        if self.freeze["status"] != "frozen_before_first_v0_3_terra_output":
            raise ValueError("v0.3 suite is not frozen for model execution")
    ...
    def preflight(self) -> None:
        ...
        if not self.resume and git_dirty():
            raise ValueError("Git worktree must be clean before a new v0.3 run")
    ...
    def build_calls(self) -> list[CallSpec]:
        ...
        for sampled in self.sample["reports"]:
            ...
            for repeat in range(1, int(self.runtime["repeats"]) + 1):
                call_dir = self.output / "calls" / document_id / f"repeat-{repeat:02d}"
                calls.append(
                    CallSpec(
                        report_id=str(sampled["report_id"]),
                        document_id=document_id,
    ...
    def call_complete(self, spec: CallSpec) -> bool:
        ...
        if not validation.is_file() or not normalized.is_file():
            return False
        return read_json(validation).get("valid") is True
        ...
        for attempt in range(1, maximum_attempts + 1):
            ...
            try:
                ...
                parsed, raw = self.provider.complete_json(
                    spec.prompt,
                    self.runtime_schema,
                    schema_name="causal_analysis_inventory_v0_3_0",
                )
                ...
                if errors:
                    ...
                    continue
                normalized = normalize_inventory(parsed, spec.atom_index)
                ...
                ledger = {
                    ...
                    "status": "valid",
                    "attempts": attempt,
                    "analysis_count": len(normalized["analyses"]),
