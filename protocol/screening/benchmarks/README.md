...
- `title_abstract_stability_holdout_v8_v0.96.0_25.csv`: evaluated exactly once
  against frozen `v0.99.0`, failed, and is now accessed evaluation evidence
  that must not be used for calibration.
...
The candidate was frozen at commit `d5646a9` and then evaluated exactly once.
The run failed with `0.96` final-route and `0.92` decisive and all-tracked
agreement; two of 25 records were unstable and one changed final route.
