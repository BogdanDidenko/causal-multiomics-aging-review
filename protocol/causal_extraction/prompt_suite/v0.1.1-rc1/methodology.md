# Causal Claim Extraction Methodology v0.1.1-rc1

## Status and purpose

This release candidate defines the post-eligibility causal claim extraction instrument for the 101 reports retained after full-text screening. It is frozen before its first model call. It does not alter study eligibility, PRISMA flow counts, or the frozen title/abstract and full-text screening instruments. Version 0.1.0-rc1 remains the immutable A0 instrument; one technical smoke window showed overgeneration of ordinary association candidates and character-inexact Markdown quotes. That output was used for this prespecified candidate-boundary and grounding refinement. Checkpoint B remained unopened.
...
Ordinary association, genetic correlation, differential expression, TWAS, colocalization-only, enrichment, and prediction outputs are evidence atoms rather than candidates unless they are themselves outputs of a named qualifying causal design. Cautious `association` wording does not disqualify an effect estimate produced by Mendelian randomization or another qualifying design.
