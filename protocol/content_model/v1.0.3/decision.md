...
named the **Causal Multi-omics Aging Content Model (CMACM) v1.0.3**.

Version 1.0.3 is a transport-only amendment to v1.0.0. It converts the dynamic
`axis_values` object into explicit `axis`/`values` records so strict JSON-schema
transport can represent WHO postcoordination without changing any scientific
entity, relation, axis, or boundary rule. It also adds explicit JSON types to
constrained string fields so Codex strict-schema transport accepts the model.
It also defines linearization membership as a controlled subset of the
Foundation, with a declared member type and selection rule.
